 ...
 *
 * Stateful by design: the caller threads a per-invoice `state` object through
 * the day-by-day simulation and calls `applyDecision` for every decision it
 * acts on, so cadence and per-tier de-dup work without recomputing history.
 */
...
/** Load and memoize config/policy.json. */
 ...
 * Call this for EVERY non-null decision so cadence and de-dup stay correct.
 */
 ...
 *   { invoice_id, date, tier_id, recipient_tier, recipient_email, auto_send,
 *     reason, held_for_human, tone, cc_tiers, cc_emails, day_offset }
 */
export function decideForInvoice(invoice, today, state = createInvoiceState(), opts = {}) {
  ...
  const lastIndex = state.lastTierId ? tiers.findIndex((t) => t.id === state.lastTierId) : -1;
  ...
  const ccEmails = ccTiers.map((r) => invoice.contacts?.[r]?.email).filter(Boolean);
